...
output_licitacoes = open("licitacoes_cafe_2013_2015.csv", "w")
output_pregoes = open("pregoes_cafe_2013_2015.csv", "w")
...
r = urllib2.urlopen("http://compras.dados.gov.br/licitacoes/v1/licitacoes.xml?data_publicacao_min=2013-01-01")
content = r.read()
...
while offset < total_offset:
    ...
    r = urllib2.urlopen("http://compras.dados.gov.br/licitacoes/v1/licitacoes.xml?data_publicacao_min=2013-01-01&offset=%d" % offset)
    content = r.read()
    tree = etree.fromstring(content)
    root = tree[1]
    resources = tree[1]
    ...
    for lic in range(len(resources)):
        ...
        for it in range(len(item_resources)):
            ...
            if licitacao.find('modalidade').text == '5' and len(item_resources) == len(item_lic_resources):
