...
output_licitacoes = open("resultado/licitacoes_busca_paracetamol_2015.csv", "w")
output_pregoes = open("resultado/pregoes_busca_paracetamol_2015.csv", "w")
...
r = urllib2.urlopen("http://compras.dados.gov.br/licitacoes/v1/licitacoes.xml?objeto=açúcar&data_publicacao_min=2014-01-01".decode('utf-8').encode('latin-1'))
...
while offset < total_offset:
    ...
    r = urllib2.urlopen("http://compras.dados.gov.br/licitacoes/v1/licitacoes.xml?objeto=açúcar&data_publicacao_min=2014-01-01&offset=%d".decode('utf-8').encode('latin-1') % offset)
    content = r.read()
    r.close()
    tree = etree.fromstring(content)
    root = tree[1]
